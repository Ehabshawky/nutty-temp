...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!adminEmailResult.success) {
      ...
      // Return a specific error if notification emails are missing in DB
      if (adminEmailResult.error === 'No notification emails configured.') {
        return NextResponse.json({ error: 'System configuration error: Notification emails not set in Admin Settings.' }, { status: 500 });
      }
      return NextResponse.json({ error: 'Failed to send application to admin. Please check SMTP settings.' }, { status: 500 });
    }
